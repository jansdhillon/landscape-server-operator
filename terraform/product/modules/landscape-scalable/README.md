...
| <a name="input_haproxy_certs"></a> [haproxy\_certs](#input\_haproxy\_certs) | Configuration for the certificates charm used by HAProxy. Set to null to skip deployment. | <pre>object({<br/>    app_name    = optional(string, "self-signed-certificates")<br/>    channel     = optional(string, "1/stable")<br/>    charm_name  = optional(string, "self-signed-certificates")<br/>    constraints = optional(string, "arch=amd64")<br/>    revision    = optional(number)<br/>    base        = optional(string, "ubuntu@24.04")<br/>  })</pre> | `{}` | no |
